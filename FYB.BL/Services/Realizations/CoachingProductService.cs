...
public class CoachingProductService : IProductService<Coaching>
{
    private readonly DataContext _context;

    public CoachingProductService(DataContext context)
    {
        _context = context;
    }
    ...
    {
        var product = await _context.Coachings
            .Include(t => t.Users)
            .FirstOrDefaultAsync(t => t.Id == productId, cancellationToken);

        if (product is null) throw new NotFoundException(ErrorMessages.CoachingNotFound);
